...
class Property:
    ...
    def showBox(self,mes=''):
            ...
            if isCancel=='no':
                ...
                return True
    def checkRound(self):
        if isMatch(templatePath="uipictures/3round.png",confid=0.98):
            self.curRound=3
            return True
        if isMatch(templatePath="uipictures/1round.png",confid=0.98):
            self.curRound=1
            return True
        if isMatch(templatePath="uipictures/2round.png",confid=0.98):
            self.curRound=2
            return True
...
def dataModeify(name="Ld9BoxHeadless.exe",data="",value=2000):
    ret=True
    proc=Pymem(name)
...
def generPassword(plaintext='',cla='B'):
    time = []
    ...
    time.append(datetime.datetime.now().strftime('%d'))
    p = plaintext + ":" + cla + (datetime.datetime.now().strftime('%Y%m%d%H'))[::-1]
...
if __name__ == '__main__':
    Hwnd = 198344
    hwnd = Hwnd
    #get_window_item_center_by_text(hwnd=Hwnd, text='雷电')
    print(matchAndClick(templatePath="uipictures/autoOff.png", confid=0.7, hWnd=hwnd,isadb=1))
    getTime()
    generMsg("Y0731C5051")
